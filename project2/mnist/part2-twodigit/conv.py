...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        next = self.conv2d_1(x)
        next= self.relu(next)
        next= self.maxpool2d(next) 
        next = self.conv2d_2(next)
        next = self.relu(next)
        next = self.maxpool2d(next)
        next = self.flatten(next)
        next = self.linear1(next)
        next = self.dropout(next)
        next = self.linear2(next)
        
        out_first_digit = next[:,:10]
        out_second_digit = next[:,10:]
